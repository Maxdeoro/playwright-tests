...
test.describe('Advanced CSS-selectors', () => {
  ...
  test('Combined search terms', async ({ page }) => {
...
test.describe('Dynamic content with conditions', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('https://osstep.github.io/locator_css');
  });
  ...
  test('Dynamic elements filtering', async ({ page }) => {
    ...
    const cheapProduct = page.locator('#dynamic-content .product-card:not(.featured) .price-value:has-text("9")');
      await expect(cheapProduct).toHaveText('9 999');
  });

  test('Combinations :has и :not', async ({ page }) => {
    // 1. Найти все карточки, которые:
    //    - Не имеют класс sold-out
    //    - Содержат кнопку с текстом "В корзину"
    // const availableProducts = page.locator('.product-cart:not(.sold-out):has(.btn:has-text("В корзину"))');
    ...
    const activeUserCells = page.locator('#user-table tr:has(.status-active td:not(:nth-child(3)))');
