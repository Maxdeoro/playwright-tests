...
test.describe('General tests by getByTitle()', () => {
  ...
  test('Get button by title', async ({ page }) => {
...
test.describe('Tests for links and special cases', () => {
  ...
  test('Get link by title', async ({ page }) => {
  ...
  test('Get abbr by title', async ({ page }) => {
...
test.describe('Complex cases and dynamic content', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('https://osstep.github.io/locator_getbytitle');
  });

  // Задание 1: Найди элемент с title содержащим пробелы в начале и конце
  test('Get element by title with spaces', async ({ page }) => {
  ...
  test('Get dynamic element', async ({ page }) => {
